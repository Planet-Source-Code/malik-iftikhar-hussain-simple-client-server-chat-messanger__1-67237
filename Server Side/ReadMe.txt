...
computer centers successfully. A complete setup file also available at my website "humsafar_ak".
...
Run:
    Run this programm only after your network connection setuped well. Must be shown all systems  in Networkneigbourhod .
...
Use Net Manager:
      First find remotsytem's name from "Messagebox"'s icon from the taskbar. Just mouse over the icon , tool tip will show the Computer name.
...
Send Message:
    1. select the remot mechine's name from the combobox's field or just type at combobox. 
     2. write the message at text box and click send button
     3. wait for reply. it will seen at recieve box, and confirm the message recieved or not.
      4.If recieve box didnot shown anything means the current mechine not communicated with remot mechine. Check network,  computername at combobox,  installed messagebox at remotemechine.
Shutdown remot mechine:
     1. select the remot mechine's name from the combobox's field or just type at combobox. 
     2. Click shutdown button and wait 
